...
export class UserRepository {
  ...
  async findAll(includeInactive = false): Promise<UserPublic[]> {
    const whereClause = includeInactive ? '' : 'WHERE is_active = true';
    const result = await query<UserPublic>(
      `SELECT id, email, name, role, is_active, created_at FROM users ${whereClause} ORDER BY created_at DESC`
    );
    return result.rows;
  }
  ...
  async create(data: CreateUserRequest): Promise<UserPublic> {
    ...
    const result = await query<UserPublic>(
      ...
      [data.email, passwordHash, data.name || null, data.role || 'admin']
    );

    return result.rows[0];
  }

  async update(id: string, data: UpdateUserRequest): Promise<UserPublic | null> {
