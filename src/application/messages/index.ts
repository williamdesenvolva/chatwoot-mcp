    ...
    switch (name) {
      case 'list_messages': {
        ...
        result = await client.get<Message[]>(
          client.accountPath(`/conversations/${conversationId}/messages`),
          params
        );
        break;
      }
      ...
      case 'create_message': {
        const conversationId = args?.conversation_id as number;
        const messageData = { ...args } as Record<string, unknown>;
      ...
      case 'delete_message': {
        ...
        await client.delete(
          client.accountPath(`/conversations/${conversationId}/messages/${messageId}`)
        );
