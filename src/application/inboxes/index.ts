...
import { Inbox } from '../../shared/types.js';
    ...
    switch (name) {
      case 'list_inboxes': {
        result = await client.get<Inbox[]>(
          client.accountPath('/inboxes')
        );
        break;
      }

      case 'create_inbox': {
        result = await client.post<Inbox>(
          client.accountPath('/inboxes'),
      ...
      case 'get_inbox': {
        const inboxId = args?.inbox_id as number;
        result = await client.get<Inbox>(
          client.accountPath(`/inboxes/${inboxId}`)
        );
        break;
      }
      ...
      case 'update_inbox': {
        ...
        result = await client.patch<Inbox>(
          client.accountPath(`/inboxes/${inboxId}`),
          updateData
        );
        break;
      ...
      case 'list_inbox_members': {
        ...
        result = await client.get(
          client.accountPath(`/inboxes/${inboxId}/members`)
        );
        break;
      }
      ...
      case 'get_inbox_agentbot': {
        const inboxId = args?.inbox_id as number;
        result = await client.get(
          client.accountPath(`/inboxes/${inboxId}/agent_bot`)
        );
        break;
      }
      ...
      case 'set_inbox_agentbot': {
        const inboxId = args?.inbox_id as number;
        result = await client.post(
          client.accountPath(`/inboxes/${inboxId}/set_agent_bot`),
          { agent_bot: args?.agent_bot_id }
        );
        break;
      }
