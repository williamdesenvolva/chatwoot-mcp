...
// Definição de tools com categoria
interface Tool {
  name: string;
  description: string;
  inputSchema: object;
...
function getToolsForScope(scope: string): Tool[] {
...
function isToolAllowed(toolName: string, scope: string): boolean {
...
async function executeTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  const accountId = args.account_id as string | number | undefined;
  if (accountId) {
  ...
  if ((path === '/' || path === '/health') && req.method === 'GET') {
    ...
    res.end(JSON.stringify({
      ...
      scopeDetails: Object.fromEntries(
        Object.entries(scopeMapping).map(([scope, categories]) => [
          scope,
          {
            categories,
