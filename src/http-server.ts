...
async function executeMcpTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    ...
    case 'search_specialist_by_name': {
      ...
      const specialists = await client.get(client.accountPath('/specialists')) as any[];
      const found = specialists.filter((s: any) =>
    ...
    case 'download_appointment_attachment': {
      ...
      const attachment = await client.get(client.accountPath(`/appointments/${args.appointment_id}/attachments/${args.attachment_id}`)) as any;
      return {
    ...
    case 'search_contact_by_phone': {
      ...
      const phoneNumber = (args.phone_number as string).replace(/\D/g, ''); // Remove não-dígitos
      const searchResult = await client.get(client.accountPath('/contacts/search'), { q: phoneNumber }) as any;

      // Filtra resultados que contenham o número de telefone
      const contacts = searchResult?.payload || searchResult || [];
      const found = Array.isArray(contacts) ? contacts.filter((c: any) => {
    ...
    case 'add_conversation_labels': {
      ...
      try {
        const conversation = await client.get(client.accountPath(`/conversations/${conversationId}`)) as any;
        const contactId = conversation?.meta?.sender?.id;
        if (contactId) {
          // Busca labels atuais do contato
          const currentContactLabels = await client.get(client.accountPath(`/contacts/${contactId}/labels`)) as any;
    ...
    case 'add_contact_labels': {
      ...
      try {
        const contactConversations = await client.get(client.accountPath(`/contacts/${contactId}/conversations`)) as any;
        ...
        for (const conv of conversations) {
          if (conv.status === 'open' || conv.status === 'pending') {
            try {
              ...
              const currentConvLabels = await client.get(client.accountPath(`/conversations/${conv.id}/labels`)) as any;
    ...
    case 'create_waitlist_entry': {
      ...
      if (args.preferred_date) entryData.preferred_date = args.preferred_date;
      if (args.preferred_time_start) entryData.preferred_time_start = args.preferred_time_start;
      if (args.preferred_time_end) entryData.preferred_time_end = args.preferred_time_end;
      if (args.notes) entryData.notes = args.notes;
      if (args.priority !== undefined) entryData.priority = args.priority;
    ...
    case 'update_waitlist_entry': {
      ...
      if (args.preferred_date) updateData.preferred_date = args.preferred_date;
      if (args.preferred_time_start) updateData.preferred_time_start = args.preferred_time_start;
      if (args.preferred_time_end) updateData.preferred_time_end = args.preferred_time_end;
    ...
    case 'accept_waitlist_slot': {
      ...
      if (args.slot_start_time) acceptData.slot_start_time = args.slot_start_time;
      if (args.slot_end_time) acceptData.slot_end_time = args.slot_end_time;
    ...
    case 'decline_waitlist_slot': {
      const entryId = args.waitlist_entry_id;
      const declineData: Record<string, unknown> = {};
      if (args.reason) declineData.reason = args.reason;
    ...
    case 'update_waitlist_notification_setting': {
      ...
      if (args.enabled !== undefined) settingData.enabled = args.enabled;
      if (args.notification_template !== undefined) settingData.notification_template = args.notification_template;
      if (args.response_timeout_hours !== undefined) settingData.response_timeout_hours = args.response_timeout_hours;
      if (args.max_notifications_per_slot !== undefined) settingData.max_notifications_per_slot = args.max_notifications_per_slot;
      if (args.auto_expire_hours !== undefined) settingData.auto_expire_hours = args.auto_expire_hours;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // Search specialist by name
      ...
      const searchName = ((params.name || params.q) as string || '').toLowerCase();
      const specialists = await client.get(client.accountPath('/specialists')) as any[];
      const found = specialists.filter((s: any) =>
        s.name.toLowerCase().includes(searchName)
      );
    ...
    // POST - Upload attachment to appointment
      ...
      const fileContent = body.file_content as string | undefined;
      const fileName = body.file_name as string | undefined;
      const fileType = body.file_type as string | undefined;
      const attachmentType = body.attachment_type as string || 'document'; // Required field
    ...
    // ============ MCP ENDPOINT (ÚNICO) - Suporta JSON-RPC e formato legado ============
      ...
      if (body.jsonrpc && body.method) {
        ...
        const response = await processMcpJsonRpc(body as {
          jsonrpc: string;
          id?: number | string;
          method: string;
          params?: Record<string, unknown>;
      ...
      switch (tool) {
        ...
        case 'download_appointment_attachment': {
          const attachmentData = await client.get(client.accountPath(`/appointments/${toolParams?.appointment_id}/attachments/${toolParams?.attachment_id}`)) as any;
          result = {
        ...
        case 'upload_appointment_attachment': {
          const apptId = toolParams?.appointment_id as number;
          const fileUrl = toolParams?.file_url as string | undefined;
          const fileContent = toolParams?.file_content as string | undefined;
          const fileName = toolParams?.file_name as string | undefined;
          const fileType = toolParams?.file_type as string | undefined;
          ...
          const uploadResult = await client.postFormData(
            client.accountPath(`/appointments/${apptId}/attachments`),
            uploadFormData
          ) as any;
        ...
        case 'create_waitlist_entry': {
          ...
          if (toolParams?.preferred_date) entryData.preferred_date = toolParams.preferred_date;
          if (toolParams?.preferred_time_start) entryData.preferred_time_start = toolParams.preferred_time_start;
          if (toolParams?.preferred_time_end) entryData.preferred_time_end = toolParams.preferred_time_end;
          if (toolParams?.notes) entryData.notes = toolParams.notes;
          if (toolParams?.priority !== undefined) entryData.priority = toolParams.priority;
        ...
        case 'update_waitlist_entry': {
          ...
          const updateData: Record<string, unknown> = {};
          if (toolParams?.preferred_date) updateData.preferred_date = toolParams.preferred_date;
          if (toolParams?.preferred_time_start) updateData.preferred_time_start = toolParams.preferred_time_start;
          if (toolParams?.preferred_time_end) updateData.preferred_time_end = toolParams.preferred_time_end;
          if (toolParams?.notes !== undefined) updateData.notes = toolParams.notes;
          if (toolParams?.priority !== undefined) updateData.priority = toolParams.priority;
        ...
        case 'accept_waitlist_slot': {
          const acceptData: Record<string, unknown> = {};
          if (toolParams?.slot_start_time) acceptData.slot_start_time = toolParams.slot_start_time;
          if (toolParams?.slot_end_time) acceptData.slot_end_time = toolParams.slot_end_time;
          result = await client.post(client.accountPath(`/appointment_waitlist_entries/${toolParams?.waitlist_entry_id}/accept`), acceptData);
          break;
        }
        case 'decline_waitlist_slot': {
          const declineData: Record<string, unknown> = {};
          if (toolParams?.reason) declineData.reason = toolParams.reason;
        ...
        case 'update_waitlist_notification_setting': {
          const settingData: Record<string, unknown> = {};
          if (toolParams?.enabled !== undefined) settingData.enabled = toolParams.enabled;
          if (toolParams?.notification_template !== undefined) settingData.notification_template = toolParams.notification_template;
          if (toolParams?.response_timeout_hours !== undefined) settingData.response_timeout_hours = toolParams.response_timeout_hours;
          if (toolParams?.max_notifications_per_slot !== undefined) settingData.max_notifications_per_slot = toolParams.max_notifications_per_slot;
          if (toolParams?.auto_expire_hours !== undefined) settingData.auto_expire_hours = toolParams.auto_expire_hours;
