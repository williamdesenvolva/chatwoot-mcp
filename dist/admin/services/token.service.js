...
export class TokenService {
    ...
    getPermissionForEndpoint(method, path) {
        ...
        for (const [pattern, permission] of Object.entries(ENDPOINT_PERMISSION_MAP)) {
            const patternRegex = new RegExp('^' + pattern.replace(/:id/g, '[^/]+').replace(/\//g, '\\/') + '$');
    ...
    hasPermission(permissions, category, action) {
    ...
    async updateToken(id, data, updatedBy) {
    ...
    async regenerateToken(id, regeneratedBy) {
    ...
    async revokeToken(id, revokedBy) {
