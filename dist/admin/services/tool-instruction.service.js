...
export class ToolInstructionService {
    async getAllToolsWithInstructions(mcpTools) {
        const customInstructions = await toolInstructionRepository.findAll();
        const instructionMap = new Map(customInstructions.map((inst) => [inst.tool_name, inst]));
    ...
    async getToolWithInstruction(toolName, mcpTools) {
    ...
    async upsertInstruction(toolName, data, userId, ipAddress, userAgent) {
    ...
    async deleteInstruction(toolName, userId, ipAddress, userAgent) {
    ...
    async getEnabledTools(mcpTools) {
        const customInstructions = await toolInstructionRepository.findAll();
        const instructionMap = new Map(customInstructions.map((inst) => [inst.tool_name, inst]));
        return mcpTools
            .filter((tool) => {
...
//# sourceMappingURL=tool-instruction.service.js.map