...
export class TokenRepository {
    async findById(id) {
        const result = await query('SELECT * FROM api_tokens WHERE id = $1', [id]);
        return result.rows[0] || null;
    }
    async findByIdWithUser(id) {
        const result = await query(`SELECT t.id, t.user_id, t.name, t.token_prefix, t.chatwoot_account_id,
    ...
    async findByHash(tokenHash) {
        const result = await query('SELECT * FROM api_tokens WHERE token_hash = $1', [tokenHash]);
        return result.rows[0] || null;
    }
    async findByPrefix(prefix) {
        const result = await query('SELECT * FROM api_tokens WHERE token_prefix = $1', [prefix]);
        return result.rows;
    }
    ...
    async create(userId, data) {
        ...
        const result = await query(`INSERT INTO api_tokens (
    ...
    async update(id, data) {
        ...
        values.push(id);
        const result = await query(`UPDATE api_tokens SET ${updates.join(', ')} WHERE id = $${paramIndex}
       RETURNING id, user_id, name, token_prefix, chatwoot_account_id,
                 permissions, rate_limit_per_minute, is_active, last_used_at,
    ...
    async regenerate(id) {
        ...
        const tokenPrefix = plainToken.substring(0, 8);
        const tokenHash = await bcrypt.hash(plainToken, 10);
        const result = await query(`UPDATE api_tokens SET token_hash = $1, token_prefix = $2, updated_at = NOW()
       WHERE id = $3
       RETURNING id, user_id, name, token_prefix, chatwoot_account_id,
                 permissions, rate_limit_per_minute, is_active, last_used_at,
                 expires_at, created_at`, [tokenHash, tokenPrefix, id]);
        if (!result.rows[0])
            return null;
        return {
            token: result.rows[0],
    ...
    async delete(id) {
        ...
        const result = await query('UPDATE api_tokens SET is_active = false, updated_at = NOW() WHERE id = $1', [id]);
        return (result.rowCount ?? 0) > 0;
    }
    async hardDelete(id) {
        const result = await query('DELETE FROM api_tokens WHERE id = $1', [id]);
        return (result.rowCount ?? 0) > 0;
    }
    ...
    async count(activeOnly = true) {
        ...
        const result = await query(`SELECT COUNT(*) as count FROM api_tokens ${whereClause}`);
