...
export async function initializeDatabase() {
    ...
    try {
        ...
        const existingUser = await client.query('SELECT id FROM users WHERE email = $1', [masterEmail]);
        if (existingUser.rows.length === 0) {
            // Import bcrypt dynamically to hash password
            const bcrypt = await import('bcrypt');
            const passwordHash = await bcrypt.hash(masterPassword, 12);
            await client.query(`INSERT INTO users (email, password_hash, name, role, is_active)
         VALUES ($1, $2, $3, $4, $5)`, [masterEmail, passwordHash, 'Master Admin', 'superadmin', true]);
