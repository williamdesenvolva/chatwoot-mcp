    ...
    try {
        ...
        switch (name) {
            case 'platform_list_agentbots': {
                ...
                result = await client.get(client.platformPath('/agent_bots'), params);
                break;
            }
            case 'platform_create_agentbot': {
                result = await client.post(client.platformPath('/agent_bots'), args);
                break;
            }
            case 'platform_get_agentbot': {
                const botId = args?.agentbot_id;
                result = await client.get(client.platformPath(`/agent_bots/${botId}`));
                break;
            }
            case 'platform_update_agentbot': {
                ...
                result = await client.patch(client.platformPath(`/agent_bots/${botId}`), updateData);
                break;
            }
