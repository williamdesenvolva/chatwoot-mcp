    ...
    try {
        ...
        switch (name) {
            case 'list_contact_labels': {
                ...
                result = await client.get(client.accountPath(`/contacts/${contactId}/labels`));
                break;
            }
            case 'add_contact_labels': {
                const contactId = args?.contact_id;
                result = await client.post(client.accountPath(`/contacts/${contactId}/labels`), { labels: args?.labels });
                break;
            }
            default:
