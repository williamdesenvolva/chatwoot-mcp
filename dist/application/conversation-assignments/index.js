    ...
    try {
        ...
        switch (name) {
            case 'assign_conversation': {
                ...
                result = await client.post(client.accountPath(`/conversations/${conversationId}/assignments`), { assignee_id: args?.assignee_id });
                break;
            }
            case 'assign_team': {
                const conversationId = args?.conversation_id;
                result = await client.post(client.accountPath(`/conversations/${conversationId}/assignments`), { team_id: args?.team_id });
                break;
            }
            default:
