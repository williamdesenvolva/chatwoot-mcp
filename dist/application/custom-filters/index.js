    ...
    try {
        ...
        switch (name) {
            case 'list_custom_filters': {
                ...
                result = await client.get(client.accountPath('/custom_filters'), params);
                break;
            }
            case 'create_custom_filter': {
                result = await client.post(client.accountPath('/custom_filters'), args);
                break;
            }
            case 'get_custom_filter': {
                const customFilterId = args?.custom_filter_id;
                result = await client.get(client.accountPath(`/custom_filters/${customFilterId}`));
                break;
            }
            case 'update_custom_filter': {
                ...
                result = await client.patch(client.accountPath(`/custom_filters/${customFilterId}`), updateData);
                break;
            }
            case 'delete_custom_filter': {
                const customFilterId = args?.custom_filter_id;
                await client.delete(client.accountPath(`/custom_filters/${customFilterId}`));
